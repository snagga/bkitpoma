...
public class BkitPoma implements EntryPoint, HistoryListener {

	private MapPanel mapPanel;
	private ToolbarButton vieButton;
	private ToolbarButton loginButton;
	private final DatabaseServiceAsync databaseService = DatabaseService.Util
			.getInstance();
	...
	public void onModuleLoad() {
		...
		loginButton.setIcon("images/openid/openidico.gif");
		...
		toolbar.addButton(new ToolbarButton(constants.insertTrackerLabel(),
		...
		toolbar.addButton(new ToolbarButton(constants.insertTrackedLabel(),
				new ButtonListenerAdapter() {
					public void onClick(Button button, EventObject e) {
						RegisterTrackedFormWindow registerTrackedForm = new RegisterTrackedFormWindow(
								"TAM1");
		 ...
		 * List of waypoint
		 */
		waypointPanel = new ListPanel(new int[] { ListPanel.FIELD_STRING,
				ListPanel.FIELD_STRING, ListPanel.FIELD_STRING,
				ListPanel.FIELD_STRING }, new String[] {
				constants.timeHeader(), constants.trackIdHeader(),
				constants.longtitudeHeader(), constants.latitudeHeader() });
		waypointPanel.setTitle(constants.waypointPanelTitle());
		// TODO sample waypoint data
		for (int i = 0; i < 10; ++i)
			waypointPanel.addRecord(new String[] { constants.timeHeader() + i,
