...
import com.bkitmobile.poma.mobile.api.connection.DeviceSession;
...
import com.bkitmobile.poma.mobile.api.util.MobileUtil;
import com.bkitmobile.poma.mobile.api.util.MidpUtil;
...
public class PomaMobile extends MIDlet implements CommandListener, LocationChangedListener {
    ...
    private DeviceSession session = new DeviceSession();
    ...
    public PomaMobile() {
        ...
        Operation.SERVICE_URL = "http://localhost:8080/api/mobile";
    }
...
//GEN-LINE:|0-initialize|1|0-postInitialize
    ...
    public void startMIDlet() {//GEN-END:|3-startMIDlet|0|3-preAction
        ...
        MidpUtil.checkMIDP();
        if (!MidpUtil.isMIDP2) {
            alertError.setString("POMA only run on MIDP 2.0+");
            switchDisplayable(null, alertError);
        } else if (!MidpUtil.isCLDC1_1) {
            alertError.setString("Your phone does not support CLDC 1.1.");
            switchDisplayable(null, alertError);
        } else if (!MidpUtil.locationSupport) {
            alertError.setString("Your phone does not support Location API.");
            switchDisplayable(null, alertError);
        }
...
//GEN-LINE:|7-commandAction|16|72-postAction
    ...
    public SplashScreen getSplashScreenPOMA() {
        if (splashScreenPOMA == null) {//GEN-END:|18-getter|0|18-preInit
            ...
            splashScreenPOMA.setText(MobileUtil.getCellId() + "," + MobileUtil.getLAC() + "," + MobileUtil.getMCC() + "," + MobileUtil.getMNC());
        ...
        if (task == null) {//GEN-END:|32-getter|0|32-preInit
            ...
            task.setExecutable(new org.netbeans.microedition.util.Executable() {
                public void execute() throws Exception {//GEN-END:|32-getter|1|32-execute
                    ...
                    session.deviceID = Long.parseLong(loginScreen.getUsername());
                    session.password = loginScreen.getPassword();

                    uid = getLoginScreen().getUsername();
                    pwd = getLoginScreen().getPassword();
                    ...
                    ConfigOperation resultConfig = session.config();
                    log(resultConfig.message);
                    if (resultConfig.ok) {
                        session.interval = locationProvider.interval = resultConfig.interval;
                        session.schedule = locationProvider.schedule = resultConfig.schedule;
    ...
    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc=" Generated Getter: alertError ">//GEN-BEGIN:|102-getter|0|102-preInit
    /**
     * Returns an initiliazed instance of alertError component.
     * @return the initialized component instance
     */
    public Alert getAlertError() {
        if (alertError == null) {//GEN-END:|102-getter|0|102-preInit
            // write pre-init user code here
            alertError = new Alert("Error", "<null>", getImage(), null);//GEN-BEGIN:|102-getter|1|102-postInit
            alertError.setTimeout(Alert.FOREVER);//GEN-END:|102-getter|1|102-postInit
            // write post-init user code here
