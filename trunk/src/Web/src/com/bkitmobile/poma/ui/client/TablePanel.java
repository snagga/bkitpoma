...
import com.bkitmobile.poma.localization.client.ListPanelConstants;
...
public class TablePanel extends LoadingPanel {
	...
	public static final int FIELD_RADIO = 3;

	private static int[] columnFieldType;
	 ...
	 */
	public TablePanel(int columnFieldType[], String columnHeader[]) {
	...
	private void constructor(int[] columnFieldType, String columnHeader[]) {
		...
		TablePanel.columnFieldType = columnFieldType;
		...
		ToolbarButton btnKMLTracked = new ToolbarButton();
		btnKMLTracked.setDisabled(true);
		btnKMLTracked.setMinWidth(0);
		pagingToolbar.addButton(btnKMLTracked);
	...
	public void setColumnWidthFitToPanel() {
		...
		int columnWidth = gridPanel.getWidth() / columnConfigs.length;
	...
	/**
	 * Creator: Tam </br> User can set Width of column by insert arr size
	 ...
	 * spaces
	 */
	public void setColumnWidthManually(int[] sizes, boolean resizable)
			throws Exception {
	...
	public void removeAllRecords() {
		...
		// store.removeAll();
		// store.load();
		proxy = new PagingMemoryProxy(new Object[1][]);
		store.setDataProxy(proxy);
		store.load();
	}
	...
	public void setTooltip(int idxColumn, String tooltip) {
		columnConfigs[idxColumn].setTooltip(tooltip);
	}
	...
	private static ColumnConfig[] getColumnConfig(String[] columnHeader) {
		...
		for (int i = 0; i < columnConfig.length; i++) {
			...
			if (columnFieldType[i] == FIELD_BOOLEAN) {
				...
				// columnConfig[i].setEditor(new GridEditor(new TextField()));
			}

			if (columnFieldType[i] == FIELD_RADIO) {
				columnConfig[i].setRenderer(new Renderer() {
					@Override
					public String render(Object value,
							CellMetadata cellMetadata, Record record,
							int rowIndex, int colNum, Store store) {
						boolean checked = ((Boolean) value).booleanValue();
						return "<input class='radio' type='radio' name='radio' "
