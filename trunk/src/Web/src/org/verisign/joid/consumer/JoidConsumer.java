...
/**
 * This is the main class for consumers to use. <p/> It performs the following
 * operations given an OpenID user identifier. - Finds the OpenId Server -
 * Associates with the server if it hasn't done so already or if the association
 * has expired - Provides url to the server an application to redirect to. <p/>
 * ... some time later ... <p/> - Takes a request from an OpenId server after
 * user has authenticated - Verifies server signature and our signatures match
 * to authenticate - Returns the user's identifier if ok <p/> <p/> User: treeder
 * Date: Jun 27, 2007 Time: 11:52:40 AM
 */
public class JoidConsumer {

	private static final Logger log = Logger.getLogger(JoidConsumer.class
			.getName());
	...
	private synchronized Properties getProps(String idserver) {
		// todo: just store the AssociationResponse instead of converting to
		// props
		...
		if (props == null) { // todo: also check expires_in time to make sure
								// it's still valid
			try {
	 ...
	 * 
	 */
	public Properties associate(String idserver) throws IOException,
			OpenIdException {
		...
		props.setProperty("publicKey", Crypto.convertToString(asr
				.getDhServerPublic()));
		props.setProperty("encryptedKey", Crypto.convertToString(asr
				.getEncryptedMacKey()));

		BigInteger privateKey = dh.getPrivateKey();
		props.setProperty("privateKey", Crypto.convertToString(privateKey));
		props.setProperty("modulus", Crypto
				.convertToString(DiffieHellman.DEFAULT_MODULUS));

		props.setProperty("_dest", idserver);
		props.setProperty("expiresIn", "" + asr.getExpiresIn());

		/*
		 * Crypto crypto = new Crypto(); dh = DiffieHellman.recreate(privateKey,
		 * p); crypto.setDiffieHellman(dh); byte[] clearKey =
		 * crypto.decryptSecret(asr.getDhServerPublic(),
	 ...
	 *            users OpenID url
	 * @param returnTo
	 *            the url to return to after user is finished with OpenId
	 *            provider
	 * @param trustRoot
	 *            base url that the authentication should apply to
	 * @return
	 * @throws OpenIdException
	 */
	 ...
	 * 
	 * @param map
	 * @return openid.identity if authentication was successful, null if
	 *         unsuccessful
	 * @throws IOException
	 * @throws OpenIdException
	 * @throws NoSuchAlgorithmException
	 */
	public AuthenticationResult authenticate(Map map) throws IOException,
			OpenIdException, NoSuchAlgorithmException {

		log.log(Level.INFO, "request map in authenticate: " + map);
		AuthenticationResponse response = new AuthenticationResponse(map);
		// todo: store nonce's to ensure we never accept the same value again -
		// see sec 11.3 of spec 2.0
		Properties props;
		if (response.getInvalidateHandle() != null) {
			// then we have to send a authentication_request (dumb mode) to
			// verify the signature
			CheckAuthenticationRequest checkReq = new CheckAuthenticationRequest(
					response.toMap(), RequestFactory.CHECK_AUTHENTICATION_MODE);
			props = getPropsByHandle(response.getInvalidateHandle());
			CheckAuthenticationResponse response2 = (CheckAuthenticationResponse) Util
					.send(checkReq, props.getProperty("idServer"));
			// todo: verify the invalidate_handle in response2 is the same as in
			// response
			removeInvalidHandle(response.getInvalidateHandle());
			if (response2.isValid()) {
				// then this is a valid request, lets send it back
				return new AuthenticationResult(response.getIdentity(),
						response);
			} else {
				throw new AuthenticationException(
						"Signature invalid, identity denied.");
			}
		} else {
			// normal properties
			props = getPropsByHandle(response.getAssociationHandle());

			// todo: before returning a valid response, ensure return_to is a
			// suburl of trust_root

			BigInteger privKey = Crypto.convertToBigIntegerFromString(props
					.getProperty("privateKey"));
			BigInteger modulus = Crypto.convertToBigIntegerFromString(props
					.getProperty("modulus"));
			BigInteger serverPublic = Crypto
					.convertToBigIntegerFromString(props
							.getProperty("publicKey"));
			byte[] encryptedKey = Crypto.convertToBytes(props
					.getProperty("encryptedKey"));

			/*
			 * String sig = response.sign(response.getAssociationType(),
			 * a.getMacKey(), response.getSignedList()); isValid =
			 * sig.equals(response.getSignature());
			 */
			...
			if (!signature.equals(reSigned)) {
				throw new AuthenticationException(
						"OpenID signatures do not match! "
